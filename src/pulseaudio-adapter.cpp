...
PulseAudioAdapter::PulseAudioAdapter() : mainloop(pa_threaded_mainloop_new()) {
    ...
    context = pa_context_new(pa_threaded_mainloop_get_api(mainloop), "touchegg_music_client");
    if (context == nullptr) {
    ...
    auto sink_info_callback = [](pa_context * /*context*/, const pa_sink_info *info, int eol, void *userdata) {
        auto *adapter = static_cast<PulseAudioAdapter *>(userdata);
    ...
    pa_operation *op = pa_context_get_sink_info_by_name(context, DEFAULT_SINK, sink_info_callback, this);
...
void PulseAudioAdapter::offset_volume(double delta_percentage) {
    ...
    auto success_callback = [](pa_context * /*context*/, int success_result, void *userdata) {
        auto *adapter = static_cast<PulseAudioAdapter *>(userdata);
        adapter->success = success_result;
        pa_threaded_mainloop_signal(adapter->mainloop, 0);
    };
    pa_operation *op = pa_context_set_sink_volume_by_index(context, sink_index, &sink_volume, success_callback, this);
    while (pa_operation_get_state(op) != PA_OPERATION_DONE) {
        pa_threaded_mainloop_wait(mainloop);
    }
    pa_operation_unref(op);
